...
export default function Trivia({ route, navigation }) {
  ...
  const getTriviaList = async (showLoader) => {
    try {
      if (showLoader) {
        dispatch(loaderAction({ isLoading: true }))
      }

      const data = await mainApi.getTriviaList({
        user_id: user?.userId,
        token,
      })
      setTriviaList(Object.values(data?.trivia))
      setData(data?.user)
      if (data?.user) {
  ...
  const renderHeaderComponent = () => {
    return (
      <View>
        ...
        >
          <Image style={{ width: 35, height: 47 }} resizeMode='contain' source={require('@assets/icons/Trivia.png')} />
          <Text style={{ fontWeight: '600', fontSize: 18, color: '#121212', marginLeft: 17 }}>Questions & Answers</Text>
        </View>
      </View>
    )
  }
  ...
  const renderItem = ({ item, index }) => {
    ...
    return (
      <TouchableOpacity
        onPress={() => navigation.navigate('TypeOfTrivia', { typeOfTrivia: item })}
      ...
      >
        <View style={{ paddingHorizontal: 17, alignItems: 'center' }}>
          ...
          <FlatList
            ...
            data={triviaList}
            keyExtractor={(item, index) => 'trivia-' + index}
